...
class EyeCareApp:
    ...
    def _on_break(self):
        ...
        THE FIX: everything between the try and the finally is best-effort.
        If a notifier raises, if the DB write fails, if the dialog throws —
        none of that is allowed to skip monitor.reset(). Before this fix,
        a single failed desktop notification meant active_seconds never
        reset, so the scheduler saw the threshold exceeded again on the
        very next tick and re-fired the break every second, forever.
        """
        ...
        try:
            ...
            logger.error("Error during break handling: %s", e)
        finally:
            self.monitor.reset()
    ...
    def _show_dialog(self):
        ...
        # Reuse the already-loaded locale messages instead of constructing
        # a second DesktopNotifier just to read its _messages dict.
        messages = self.desktop_notifier._messages
        ...
        frame.pack(pady=10)
        tk.Button(frame, text="Dismiss (Done)", command=on_dismiss, width=12).pack(side=tk.LEFT, padx=5)
        tk.Button(frame, text="Snooze (1 min)", command=on_snooze, width=12).pack(side=tk.RIGHT, padx=5)
